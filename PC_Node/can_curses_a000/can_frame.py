...
class CanTransmitFrame:
    ...
    def __init__(self, frame_dict, can_globals, can_dbc) -> None:
        ...
        #
        #timers
        self._timer_68h = IntervalTimer(interval=self._frame_info[0x68]['cycle_time'], first_immediately=True, timer_name="Can_Tx_Object_Timer_0x68", context={'globals': can_globals, 'dbc': can_dbc}, callback=self.callback_68h)
        self._timer_3h = IntervalTimer(interval=self._frame_info[0x3]['cycle_time'], first_immediately=True, timer_name="Can_Tx_Object_Timer_0x3", context={'globals': can_globals, 'dbc': can_dbc}, callback=self.callback_3h)
        self._timer_6dh = IntervalTimer(interval=self._frame_info[0x6D]['cycle_time'], first_immediately=True, timer_name="Can_Tx_Object_Timer_0x6D", context={'globals': can_globals, 'dbc': can_dbc}, callback=self.callback_6dh)
        #


    def update_68h(self, can_globals, can_dbc) -> None:

        self._frame_info[0x68].update({'sig_payload': can_globals.tx_frame_info[0x68]['sig_payload']})
        self._frame_info[0x68].update({'byte_payload': can_globals.tx_frame_info[0x68]['byte_payload']})

        for signal in can_dbc.ListSignalsByFrame(0x68):

            if self._frame_info[0x68]['sig_payload'][signal] > can_dbc.GetSignalMaximum(signal):
                self._frame_info[0x68]['sig_payload'].update({signal: can_dbc.GetSignalMaximum(signal)})

            if self._frame_info[0x68]['sig_payload'][signal] < can_dbc.GetSignalMinimum(signal):
                self._frame_info[0x68]['sig_payload'].update({signal: can_dbc.GetSignalMinimum(signal)})

        can_globals.tx_frame_info.update({0x68: self._frame_info[0x68]})


    def send_68h(self, can_globals, can_dbc) -> None:

        if self._frame_info[0x68]['is_resolved'] == True:

            encoded_tx_payload = can_dbc._db.encode_message(0x68, self._frame_info[0x68]['sig_payload'], strict=False)

        else:

            encoded_tx_payload = bytearray(self._frame_info[0x68]['byte_payload'])

        tx_frame = Frame(id_=0x68, data=encoded_tx_payload, flags=canlib.MessageFlag.STD)

        can_globals.com_port.write(tx_frame)


    def update_3h(self, can_globals, can_dbc) -> None:

        self._frame_info[0x3].update({'sig_payload': can_globals.tx_frame_info[0x3]['sig_payload']})
        self._frame_info[0x3].update({'byte_payload': can_globals.tx_frame_info[0x3]['byte_payload']})


    def send_3h(self, can_globals, can_dbc) -> None:

        if self._frame_info[0x3]['is_resolved'] == True:

            encoded_tx_payload = can_dbc._db.encode_message(0x3, self._frame_info[0x3]['sig_payload'], strict=False)

        else:

            encoded_tx_payload = bytearray(self._frame_info[0x3]['byte_payload'])

        tx_frame = Frame(id_=0x3, data=encoded_tx_payload, flags=canlib.MessageFlag.STD)

        can_globals.com_port.write(tx_frame)


    def update_6dh(self, can_globals, can_dbc) -> None:

        self._frame_info[0x6d].update({'sig_payload': can_globals.tx_frame_info[0x6d]['sig_payload']})
        self._frame_info[0x6d].update({'byte_payload': can_globals.tx_frame_info[0x6d]['byte_payload']})


    def send_6dh(self, can_globals, can_dbc) -> None:

        if self._frame_info[0x6d]['is_resolved'] == True:

            encoded_tx_payload = can_dbc._db.encode_message(0x6d, self._frame_info[0x6d]['sig_payload'], strict=False)

        else:

            encoded_tx_payload = bytearray(self._frame_info[0x6d]['byte_payload'])

        tx_frame = Frame(id_=0x6d, data=encoded_tx_payload, flags=canlib.MessageFlag.STD)

        can_globals.com_port.write(tx_frame)


    async def callback_68h(self, timer_name, context, timer) -> None:
        await asyncio.sleep(0.001)
        self.update_68h(context['globals'], context['dbc'])
        self.send_68h(context['globals'], context['dbc'])


    async def callback_3h(self, timer_name, context, timer) -> None:
        await asyncio.sleep(0.001)
        self.update_3h(context['globals'], context['dbc'])
        self.send_3h(context['globals'], context['dbc'])


    async def callback_6dh(self, timer_name, context, timer) -> None:
        await asyncio.sleep(0.001)
        self.update_6dh(context['globals'], context['dbc'])
        self.send_6dh(context['globals'], context['dbc'])
